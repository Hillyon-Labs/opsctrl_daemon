...
exports.diagnosePod = diagnosePod;
exports.getPodEvents = getPodEvents;
exports.getContainerLogs = getContainerLogs;
exports.runLocalDiagnosis = runLocalDiagnosis;
exports.diagnoseStack = diagnoseStack;
exports.getStackDataForBackend = getStackDataForBackend;
exports.loadAllRules = loadAllRules;
exports.sanitizeLogs = sanitizeLogs;
exports.handlePreliminaryDiagnostic = handlePreliminaryDiagnostic;
 ...
 */
async function diagnosePod(podName, namespace, _container) {
    // Always use stack-based diagnosis for better coverage
    console.log(chalk_1.default.blue('🔄 Redirecting to comprehensive stack analysis...\n'));
    return await diagnoseStack(podName, namespace);
}
async function getPodStatus(podName, namespace) {
    const coreV1 = (0, kube_1.getCoreV1)();
    try {
        ...
        initContainers.forEach((initContainer) => {
            containerStates.push((0, utils_1.parseContainerState)(initContainer, 'init'));
        });
        mainContainers.forEach((mainContainer) => {
            containerStates.push((0, utils_1.parseContainerState)(mainContainer, 'main'));
        });
        ...
        (0, utils_1.printErrorAndExit)(message);
    }
}
/**
 ...
 */
async function getPodEvents(podName, namespace) {
    const coreV1 = (0, kube_1.getCoreV1)();
    try {
        ...
        console.log(err);
        console.error(`\n ${chalk_1.default.red(`Error fetching events for pod ${podName}`)}`);
        (0, utils_1.printErrorAndExit)(`Error fetching events for pod ${podName}`);
    }
}
/**
 ...
 * @returns
 */
async function getContainerLogs(podName, namespace, container, tailLines = 200) {
    const coreV1 = (0, kube_1.getCoreV1)();
    try {
        ...
        const logPromises = allContainers.map(async ({ name, type }) => {
            try {
                ...
                console.log(chalk_1.default.red(`[${name}] log fetch failed — ${message}`));
...
function runLocalDiagnosis(containerStates, events, logs) {
    const ruleFiles = loadAllRules(); // JSON objects from rules folder
    for (const rule of ruleFiles) {
        const matchingContainerState = rule.match.containerStates?.some((state) => containerStates.some((cs) => cs.state.includes(state)));
        const matchLogs = rule.match.logs?.some((matcher) => logs.some((line) => matchLine(line, matcher)));
        const matchEvents = rule.match.events?.some((matcher) => events.some((line) => matchLine(line, matcher)));
 ...
 * @returns
 */
async function diagnoseStack(podName, namespace) {
 ...
 * @returns
 */
async function extractHelmRelease(podName, namespace) {
    const coreV1 = (0, kube_1.getCoreV1)();
    try {
        const pod = await coreV1.readNamespacedPod({ name: podName, namespace });
 ...
 * @returns
 */
async function findStackComponents(releaseName, namespace, originalPod) {
    const coreV1 = (0, kube_1.getCoreV1)();
    try {
...
async function collectStackDiagnostics(pods, namespace) {
 ...
 * @returns
 */
function analyzeStackLocally(stackDiagnostics, releaseName, primaryPod) {
    const failedComponents = stackDiagnostics.filter(component => component.status.phase !== 'Running' &&
        component.status.containerStates.some(state => !state.state.startsWith('Running') && !state.state.startsWith('Terminated: Completed')));
    const healthyComponents = stackDiagnostics.filter(component => component.status.phase === 'Running' &&
        component.status.containerStates.every(state => state.state.startsWith('Running') || state.state.startsWith('Terminated: Completed')));
    ...
    if (failedComponents.length > 0) {
        ...
        failedComponents.forEach(component => {
            ...
            const failedContainers = component.status.containerStates.filter(state => !state.state.startsWith('Running') && !state.state.startsWith('Terminated: Completed'));
 ...
 */
async function getStackDataForBackend(podName, namespace) {
    try {
 ...
 */
function detectCommonFailures(failedComponents) {
    const issues = [];
    // Check for image pull failures
    const imagePullFailures = failedComponents.filter(component => component.events.some(event => event.includes('ImagePull')) ||
    ...
    const resourceIssues = failedComponents.filter(component => component.events.some(event => event.includes('Insufficient') || event.includes('OutOfMemory') || event.includes('OutOfCPU')));
    if (resourceIssues.length > 0) {
        issues.push(`Resource constraints affecting ${resourceIssues.length} components`);
    }
    // Check for configuration issues
    const configIssues = failedComponents.filter(component => component.events.some(event => event.includes('ConfigMap') || event.includes('Secret') || event.includes('Mount')));
 ...
 */
function analyzeDependencies(stackDiagnostics, _releaseName) {
    const issues = [];
    // Look for database/service dependencies
    const serviceComponents = stackDiagnostics.filter(component => component.podName.includes('db') ||
        component.podName.includes('database') ||
        component.podName.includes('redis') ||
        component.podName.includes('cache'));
    const failedServices = serviceComponents.filter(component => component.status.phase !== 'Running');
    if (failedServices.length > 0) {
        const dependentComponents = stackDiagnostics.filter(component => !serviceComponents.includes(component) &&
...
function handlePreliminaryDiagnostic(match) {
